# Aubade

朝の行動を短時間で記録し、週次で振り返るための個人用PWA。
依存ゼロの単一HTMLファイルで、記録は端末の `localStorage` にのみ残ります。

**→ https://yugetayuki.github.io/flourish-log/**

## これは何をするアプリか

**行動を起こさせる装置ではなく、計測の装置です。** 行動変容の主役は環境設計（アプリの外）にあり、
このアプリの役割は (a) 未定着の行動が定着するまでの足場、(b) 週次フィードバックループの計測層、
の2つに限定しています。

そのため、意図的に持っていない機能があります。

- **連続日数（ストリーク）を表示しない** — 主指標は週次達成率（例 3/5）。
  完璧主義傾向の強い利用者にとって、ストリークは断絶時の "what-the-hell effect" を誘発しやすいため
- **欠損を失敗として扱わない** — 未入力（中空のドット）と未達（塗りつぶし）を区別する
- **バッジ・紙吹雪・通知・ランキングを持たない** — 反ドーパミン設計
- **残数カウントや督促を出さない** — 事実の提示は可、行動の指示は不可

## 記録するもの

| 時点 | 項目 |
|---|---|
| 昨夜 | 就寝時刻（15分刻み） / アシュワガンダ |
| 今朝 | 起床時刻（15分刻み・計測のみ・目標にしない） / 眠れた感（同上） / 朝コーヒー / クレアチン / 体重測定 |
| 昨日 | YouTube視聴時間（30分刻み） / 歩数（1000歩刻み） / 水分（500ml刻み） / ジム / サウナ / お風呂・肌ケア / マウスケア / タンパク質 / 勉強 / 瞑想 / 休肝日 / 任意のカスタム項目 |
| 仕事 | 業務時間（30分刻み） / 仕事の重さ（軽い・普通・重い）— どちらも計測のみ・目標を持たない |
| 当日 | 食事の節制（朝昼晩） / 整腸剤・サプリ（朝昼晩） / 今の気分（朝昼晩・高/中/低、計測のみ） |

タブは 記録 / 週 / 推移 / 週報 / 設定 の5つ。週報タブが出力するテキストを
Claude に貼り付けると、【事実】【解釈】【来週の一点】の形式でレビューが返る運用です。

## プライバシー

- 記録は端末のブラウザ内（`localStorage`）に保存されます
- **送信先は「オーナー自身のPC」1つだけ**です（v2.8 で追加。設定タブで送信先とトークンを
  入れたときにだけ働き、既定は空欄＝何も送りません）。送り先は Tailscale の `*.ts.net` に
  限られ、CSP（`connect-src https://*.ts.net`）がそれ以外への通信を遮断します
- それ以外の外部送信はありません。fetch は同期の1本だけで、`XMLHttpRequest` /
  `WebSocket` / `sendBeacon` は使っていません。実ブラウザのテストで
  「許可した宛先以外へは0件しか届かない」ことを毎回確認しています
- 送信先URLとトークンはコードに書かれておらず、このリポジトリにも含まれません
- このリポジトリに実データは一切含まれません

サイト自体は URL を知っていれば誰でも開けますが、開いても中身は空のアプリです。

## 技術

- 出荷物は `index.html` と `sw.js` の2本だけ。vanilla JS + インラインCSS + インラインSVG。
  フレームワーク・CDN・ビルド工程なし（オフライン耐性と検証容易性のための選択）。
  `sw.js` は Service Worker が別ファイルでしか登録できない仕様上の分割
- 保存: `localStorage`、即時書き込み
- ホスティング: GitHub Pages（`main` の `/`）

```bash
npm install
npm test        # 実HTMLをJSDOMで起動して操作する（本数は pwa.test.js が正）
npm run smoke   # 実ブラウザ(Chromium) — 描画・保存・CSP・Service Worker の実効性
```

設計判断の背景、不変条件、バックログ、「再訪禁止リスト」は [CLAUDE.md](CLAUDE.md) にまとめてあります。
変更を加えるときは先にそちらを読んでください。
