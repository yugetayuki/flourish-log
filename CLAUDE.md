...
- 最終更新: 2026-08-09 / 現行バージョン: **v2.6**(`index.html` 内 eyebrow 表記と一致させること)
- テスト: `npm install && npm test`(vitest 98本 + 実ブラウザ smoke 22項目、全パス)
...
### データモデル(localStorage `flourish-log-v2`、schema v5)
...
- **entriesの意味論(重要)**: キーの日付=「記録した朝」。フィールドが指す時点は
  ...
  前日(youtube, gym, study, sauna, steps, sober, カスタム)/ **当日(mealB, mealL, mealD)**。
  **カスタム項目は必ず前日を指す**(「昨日」カードに描画される)。当朝・前夜の行動を足すときは `CORE` 側に置くこと。
- **食事(mealB/mealL/mealD)だけが当日を指す(v2.6)。** 朝にまとめて入力する他の項目と違い、
  食べたその場で1タップする運用なので、記録した朝ではなくその日自身を指す。
  週タブと目標では3行に増やさず、`achieved()` が1日の `meal` に畳む
  (**記録した食事がすべて節制なら達成**、1つも記録がなければ未入力)。
  記録しなかった食事を分母に入れない理由は設計制約2(欠損は失敗ではない)。
...
```bash
npm install
npm test        # vitest 98本。ロジックとDOM操作
