...
- テスト: `npm install && npm test`(vitest 117本 + 実ブラウザ smoke 25項目、全パス)
...
```bash
npm install
npm test        # vitest 117本。ロジックとDOM操作
...
- 緩和先が持ち出しに使えないこと。`*.ts.net` は Tailscale の内部ドメインで、
  **tailnet に参加している端末からしか名前を引けない**。攻撃者のサーバーをそこに置くことはできない。
