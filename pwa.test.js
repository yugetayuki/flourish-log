...
describe("PWA v4.3: 起動と基本描画", () => {
  it("記録タブが描画され、v4.3表示がある", () => {
    const dom = boot();
    expect(q(dom, "#view").textContent).toContain("就寝時刻");
    expect(q(dom, ".eyebrow").textContent).toContain("v4.3");
  });
...
describe("PWA v4.3: 壊れた保存データを黙って消さない", () => {
...
describe("PWA v4.3: Service Worker", () => {
...
describe("PWA v4.3: 取り込んだJSONを信用しない", () => {
...
describe("PWA v4.3: ひとこと(その日の出来事)", () => {
...
describe("PWA v4.3: 誰と過ごしたか(計測のみ)", () => {
...
describe("PWA v4.3: 同期の取り扱いを壊さない", () => {
  const SYNC_KEY = "flourish-log-v2-sync";
  const URL_OK = "https://pc.example-tailnet.ts.net/aubade";
  const stubFetch = (dom, impl) => {
    const calls = [];
...
describe("PWA v4.3: v7 スキーマ(歩数の4段階化)", () => {
...
describe("PWA v4.3: 項目の結線ガード(CORE 起点)", () => {
...
describe("PWA v4.3: v9 スキーマ(就寝・起床・YouTube の分値化)", () => {
...
describe("PWA v4.3: お風呂・肌ケア / マウスケア", () => {
  const today = (f) => f.fmt(new Date());
  const ITEMS = [
