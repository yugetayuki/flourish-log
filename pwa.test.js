...
// describe 名に版(v4.21 など)を書かない。リリースのたびに数十箇所を手で直すことになり、
// 直し忘れても何も落ちないので必ず腐る。版の真実は package.json と index.html の eyebrow で、
// schema の真実は defaultData().version(下の CURRENT_SCHEMA が導出する)。
// 見出しの「(v18)」「(v22)」は「どの schema で入った項目か」であって版表記ではないので残す。
...
describe("PWA: 勉強を分の実値へ移す(study → studyMin)", () => {
...
describe("PWA: 歩数の移行(v7 の4段階化 → v16 の実値化)", () => {
...
describe("PWA: v9 スキーマ(就寝・起床・YouTube の分値化)", () => {
...
describe("PWA: 水分を当日へ移す(v18)", () => {
  ...
  it("明日ぶんを開くと当日カードの見出しが「明日の〜」になる", () => {
    ...
    // **先頭が「昨夜」なのは v4.28 のオーナー判断**(就寝時刻へ毎朝到達する距離を、
    // v4.24 の時系列順より優先した。頻度は実測ではなくオーナーの申告)。
    // 時系列(昨日→昨夜)へ戻すのは蒸し返しに当たる
    expect(titles()).toEqual(["昨夜", "昨日", "今朝", "今日の水分", "今日の昼寝", "今日の食事", "今日のサプリ", "今日の気分"]);
    qa(dom, "[data-dn]").find((b) => b.dataset.dn === "1").click();
    expect(titles()).toEqual(["今夜", "今日", "明日の朝", "明日の水分", "明日の昼寝", "明日の食事", "明日のサプリ", "明日の気分"]);
  });
