...
describe("PWA v4.18: 睡眠時間(就寝と起床から導く)", () => {
  const sleepRow = (dom) =>
  ...
  it("行は「眠れた感」より下に置く", () => {
    ...
    expect(labels.indexOf("睡眠時間")).toBeGreaterThan(labels.indexOf("眠れた感"));
    expect(labels.indexOf("睡眠時間")).toBeGreaterThan(labels.indexOf("起床時刻"));
  });
