...
describe("PWA v3.1: 起動と基本描画", () => {
  it("記録タブが描画され、v3.1表示がある", () => {
    const dom = boot();
    expect(q(dom, "#view").textContent).toContain("就寝時刻");
    expect(q(dom, ".eyebrow").textContent).toContain("v3.1");
  });
...
describe("PWA v3.1: 壊れた保存データを黙って消さない", () => {
...
describe("PWA v3.1: Service Worker", () => {
...
describe("PWA v3.1: 取り込んだJSONを信用しない", () => {
...
describe("PWA v3.1: 同期の取り扱いを壊さない", () => {
  const SYNC_KEY = "flourish-log-v2-sync";
  const URL_OK = "https://pc.example-tailnet.ts.net/aubade";
  const stubFetch = (dom, impl) => {
    const calls = [];
...
describe("PWA v3.1: v7 スキーマ(歩数の4段階化)", () => {
...
describe("PWA v3.1: 項目の結線ガード(CORE 起点)", () => {
